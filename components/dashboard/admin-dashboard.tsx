...
export async function AdminDashboard({ adminId }: { adminId: string }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* KPI Row — static cards, no dynamic Tailwind */}
            ...
            <div className="space-y-2">
              {recentTrips.map((t: any) => (
                <div key={t.id} className="flex items-center gap-3 rounded-lg p-2 hover:bg-slate-900">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-slate-300 truncate">{t.source} → {t.destination}</p>
                  </div>
                  <span className={`shrink-0 text-[10px] font-semibold ${statusTextClass(t.status)}`}>
                    {t.status}
                  </span>
                </div>
              ))}
